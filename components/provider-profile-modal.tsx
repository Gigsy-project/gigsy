...
import { Star, MapPin, Briefcase, Globe, Shield, Calendar, MessageCircle, Heart } from "lucide-react"
...
export function ProviderProfileModal({ provider, isOpen, onClose, onRemoveFavorite }: ProviderProfileModalProps) {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      router.push(`/messages?contact=${encodeURIComponent(provider.name)}&providerId=${provider.id}`)
    } catch (error) {
  ...
  const stats = [
    ...
    {
      number: provider.joinDate ? new Date().getFullYear() - new Date(provider.joinDate).getFullYear() : 2,
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-6">
            <h2 className="text-2xl font-bold">Información sobre mí</h2>
          ...
          <div className="space-y-6 mb-8">
            <div className="flex items-center gap-4 p-4 rounded-lg border">
              ...
              <div>
                ...
                <span>{provider.description || "Servicios profesionales de calidad"}</span>
              </div>
            </div>
            ...
            {provider.verified && (
              <div className="flex items-center gap-4 p-4 rounded-lg border">
                ...
                <div>
                  <span className="font-medium underline text-green-600">Identidad verificada</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground">
              Último servicio: {provider.lastService} - {provider.lastServiceDate}
            </p>
          </div>
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <Button variant="outline" className="flex-1" onClick={() => setShowBookingModal(true)}>
            ...
            <Button
              ...
              disabled={isRemoving}
            >
              <Heart className={`h-5 w-5 ${isRemoving ? "animate-pulse" : "fill-current"}`} />
            </Button>
          </div>
        </DialogContent>
      </Dialog>
